(******************************************************************************)
(*                                                                            *)
(*     Sikora-Weller kinetic instance and the Boltzmann collision model       *)
(*                                                                            *)
(*     Instantiates KINETIC_MODEL_PARAMS using a constant cross-section bound *)
(*     derived from the Sikora-Weller p-11B tabulation and a                  *)
...
(* ================================================================== *)
(* === Boltzmann transport and the BGK collision model === *)
(* ================================================================== *)

(* Beyond the bounded kinetic instance above, the velocity-space
   transport equation carries mass/energy moments, the BGK relaxation
   operator with its conservation laws and quadratic H-theorem, and the
   binary collision integral with the gain-loss conservation laws. *)
...
(* RInt_scal_R is provided by pb_avalanche_integral. *)
...
(* === BGK relaxation dynamics ===
   ...
   It solves the ODE exactly, and the deviation from equilibrium decays
...
(* Quadratic entropy: the squared L^2 distance to equilibrium. The
   Boltzmann H-functional uses f ln(f/f_eq); the convex quadratic
   surrogate is a Lyapunov functional for the linear BGK relaxation and
   obeys the same monotone-decay law. *)
